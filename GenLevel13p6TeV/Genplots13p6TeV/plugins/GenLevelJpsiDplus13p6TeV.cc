// -*- C++ -*-
//
// Package:    GenTutorial/GenLevelJpsiDplus13p6TeV
// Class:      GenLevelJpsiDplus13p6TeV
//
/**\class GenLevelJpsiDplus13p6TeV GenLevelJpsiDplus13p6TeV.cc GenTutorial/GenLevelJpsiDplus13p6TeV/plugins/GenLevelJpsiDplus13p6TeV.cc
...
class GenLevelJpsiDplus13p6TeV : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
	public:
		explicit GenLevelJpsiDplus13p6TeV(const edm::ParameterSet&);
		~GenLevelJpsiDplus13p6TeV();
...
////////////////////////////////////////////////////////////////////////////
GenLevelJpsiDplus13p6TeV::GenLevelJpsiDplus13p6TeV(const edm::ParameterSet& iConfig)
...
GenLevelJpsiDplus13p6TeV::~GenLevelJpsiDplus13p6TeV()
{
...
/////////////////////
void GenLevelJpsiDplus13p6TeV::beginJob()
{
    // Open a ROOT file to store the output
    outFile_ = new TFile("control_plots_jpsidplus_13p6TeV.root", "RECREATE");
...
void GenLevelJpsiDplus13p6TeV::initialize()
{
...
void GenLevelJpsiDplus13p6TeV::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	using namespace edm;
	using namespace std;
	...
	if (genParticles.isValid()) {
		...
		for (const auto& genParticle : *genParticles) {
            ...
            //if (std::abs(genParticle->pdg_id()) == 411) {
			if (abs(genParticle.pdgId()) == 411) {
				
				TLorentzVector dplus;
                
				dplus.SetPtEtaPhiM(genParticle.pt(), genParticle.eta(), genParticle.phi(), genParticle.mass());
                ...
                float phi = dplus.Phi();
				    ...
				    if ((pt >= 4.0 && pt <= 60.0) && (abs(rapidity) <= 2.1) ) {
						...
						//dplusVertices.push_back(genParticle->production_vertex());
                }
		...
		for (size_t i=0; i < jpsiParticles.size(); ++i){
			...
			for (size_t j=0; j < dplusParticles.size(); ++j){
				...
				genJpsidplus_delta_phi_.push_back(abs(jpsiParticles[i].Phi() - dplusParticles[j].Phi()));
...
///////
//++++++++++++++++++
void GenLevelJpsiDplus13p6TeV::endJob(){
...
DEFINE_FWK_MODULE(GenLevelJpsiDplus13p6TeV);
