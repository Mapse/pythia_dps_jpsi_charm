// -*- C++ -*-
//
// Package:    GenTutorial/GenLevelJpsiDsplus13p6TeV
// Class:      GenLevelJpsiDsplus13p6TeV
//
/**\class GenLevelJpsiDsplus13p6TeV GenLevelJpsiDsplus13p6TeV.cc GenTutorial/GenLevelJpsiDsplus13p6TeV/plugins/GenLevelJpsiDsplus13p6TeV.cc
...
class GenLevelJpsiDsplus13p6TeV : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
	public:
		explicit GenLevelJpsiDsplus13p6TeV(const edm::ParameterSet&);
		~GenLevelJpsiDsplus13p6TeV();
...
////////////////////////////////////////////////////////////////////////////
GenLevelJpsiDsplus13p6TeV::GenLevelJpsiDsplus13p6TeV(const edm::ParameterSet& iConfig)
...
GenLevelJpsiDsplus13p6TeV::~GenLevelJpsiDsplus13p6TeV()
{
...
/////////////////////
void GenLevelJpsiDsplus13p6TeV::beginJob()
{
    // Open a ROOT file to store the output
    outFile_ = new TFile("control_plots_jpsidsplus_13p6TeV.root", "RECREATE");
...
void GenLevelJpsiDsplus13p6TeV::initialize()
{
...
void GenLevelJpsiDsplus13p6TeV::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	using namespace edm;
	using namespace std;
	...
	if (genParticles.isValid()) {
		...
		for (const auto& genParticle : *genParticles) {
            ...
            //if (std::abs(genParticle->pdg_id()) == 431) {
			if (abs(genParticle.pdgId()) == 431) {
				
				TLorentzVector dsplus;
                
				dsplus.SetPtEtaPhiM(genParticle.pt(), genParticle.eta(), genParticle.phi(), genParticle.mass());
                ...
                float phi = dsplus.Phi();
				    ...
				    if ((pt >= 4.0 && pt <= 60.0) && (abs(rapidity) <= 2.1) ) {
						...
						//dsplusVertices.push_back(genParticle->production_vertex());
                }
...
///////
//++++++++++++++++++
void GenLevelJpsiDsplus13p6TeV::endJob(){
...
DEFINE_FWK_MODULE(GenLevelJpsiDsplus13p6TeV);
