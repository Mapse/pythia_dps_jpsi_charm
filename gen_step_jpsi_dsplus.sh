...
if [ $# -ne 2 ]; then
    echo "Usage: source gen_step_jpsi_dplus.sh <EnergyTag> <NumberOfEvents>"
    echo "Example: source gen_step_jpsi_dsplus.sh 13TeV 600000"
fi

# Get energy from argumentss
energy=$1
...
## Path to gs frameng
path_gs=Configuration/GenProduction/python/Jpsi_Dsplus_DPS_${energy}_cfi.py

# GS cfg fragment 
py_gs=Jpsi_Dsplus_DPS_${energy}_GS_cfg.py
# For MC Gen parameters 
REPORT_NAME=Jpsi_Dsplus_DPS_${energy}_report.xml

# GS root
root_gs=Jpsi_Dsplus_DPS_${energy}.root
...
# Run 3 (13.6 TeV): using 2022 as base  
